...
| Tool | Common Usage |
|------|--------------|
| `mcp__contextstream__search` | `mcp__contextstream__search(mode="semantic", query="...", limit=3)` — modes: semantic, hybrid, keyword, pattern |
...
- **Search defaults**: `mcp__contextstream__search` returns top 3 results with compact snippets; use `limit`, `offset`, and `content_max_chars` to expand
