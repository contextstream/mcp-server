...
const ENABLE_JSON_RESPONSE =
  (process.env.MCP_HTTP_JSON_RESPONSE || "false").toLowerCase() === "true";
...
const WELL_KNOWN_CONFIG_PATHS = new Set([WELL_KNOWN_CONFIG_PATH, "/.well-known/mcp-config.json"]);
const WELL_KNOWN_CARD_PATHS = new Set(["/.well-known/mcp.json", "/.well-known/mcp-server.json"]);
...
function extractAuthOverride(
  req: IncomingMessage,
  fallback: AuthOverride | null
): { override: AuthOverride | null; tokenType?: "api_key" | "jwt" } {
  const apiKey =
  ...
  const workspaceId =
    normalizeHeaderValue(req.headers["x-contextstream-workspace-id"]) ||
    normalizeHeaderValue(req.headers["x-workspace-id"]);
  const projectId =
    normalizeHeaderValue(req.headers["x-contextstream-project-id"]) ||
  ...
  return {
    ...
    properties: {
      apiKey: {
        ...
        description:
          "Optional ContextStream API key or JWT (required for authenticated tool calls).",
      },
      workspaceId: {
        type: "string",
        title: "Workspace ID",
...
async function handleMcpRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (REQUIRE_AUTH && !token) {
    writeJson(res, 401, {
      error: "Missing authorization token. Use Authorization: Bearer <API_KEY> or X-API-Key.",
    });
    return;
  }
...
export async function runHttpGateway(): Promise<void> {
  ...
  console.error(
    `[ContextStream] Auth: ${config.apiKey ? "API Key" : config.jwt ? "JWT" : "Header-based"}`
  );
  ...
  server.listen(PORT, HOST, () => {
    console.error(
      `[ContextStream] MCP HTTP gateway listening on http://${HOST}:${PORT}${MCP_PATH}`
    );
  });
}
